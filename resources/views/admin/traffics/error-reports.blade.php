...
@section('content')
<div class="col-12 card  row p-4" style="padding: 30px 0px;position: relative;{{ $settings['dashboard_dark_mode'] != '1'?'background: #fff':'' }};overflow: auto;">
    <table id="myTable" class="table table-striped table-bordered col-12 " style="padding: 0px;min-width: 1500px;">
        ...
        <tbody>
            ...
            <tr>
                ...
                <th style="font-size: 12px;width:30%;white-space: break-spaces;">{{$report->title}}</th>
                <th style="font-size: 12px">{{$report->code}}</th>
                <th style="font-size: 12px">
                    <a href="{{$report->url}}">{{substr($report->url, 0, 30)}}</a>
                </th>
                <th style="font-size: 12px">{{$report->ip}}</th>
                <th style="font-size: 12px;text-align:center;background: {{strpos($report->user_agent,'bot')==false?'#e6e6e6':'#fff'}}">
